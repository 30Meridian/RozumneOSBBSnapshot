...
class LastTopicsFeed(Feed):
    ...
    def get_object(self, request, *args, **kwargs):
        self.condominium_slug = Condominium.objects.filter(slug=request.session['condominium_slug']).values_list('slug', flat=True)
        print (self.condominium_slug[0])
        ...
        return self.condominium_slug

    def link(self,obj):
        return reverse_lazy('condominium:forum:index', kwargs={'condominium_slug': obj[0]})

    def items(self):
        return Topic.objects.filter(forum__in=self.forums, approved=True).order_by('-last_post_on')

    def item_link(self, item):
        return reverse_lazy('condominium:forum_conversation:topic', kwargs={
            'forum_slug': item.forum.slug, 'forum_pk': item.forum.pk,
            'slug': item.slug, 'pk': item.id,
            'condominium_slug': self.condominium_slug[0]})
