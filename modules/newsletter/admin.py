...
class NewsletterAdmin(admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        obj.condominium = Condominium.objects.get(id=request.session['condominium_id'])
        obj.save()

    def get_queryset(self, request):
        if request.user.is_superuser:
            return super(NewsletterAdmin, self).get_queryset(request)
        else:
...
class SubmissionAdmin(admin.ModelAdmin, ExtendibleModelAdminMixin):
    ...
    def get_queryset(self, request):
        if request.user.is_superuser:
            return super(SubmissionAdmin, self).get_queryset(request)
        else:
            qs = super(SubmissionAdmin, self).get_queryset(request)
            return qs.filter(newsletter__condominium=request.session['condominium_id'])
    """ List extensions """
    ...
    def admin_publish_date(self, obj):
        if obj.publish_date:
            return '{}{}'.format(_(datetime.strftime(obj.publish_date, '%b')),
                                     datetime.strftime(obj.publish_date, '.%d, %Y, %H:%M'))
        else:
            return ''
...
class MessageForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['newsletter'].queryset = Newsletter.objects.filter(condominium=self.current_condominium)
...
class MessageAdmin(admin.ModelAdmin, ExtendibleModelAdminMixin):
    ...
    def get_queryset(self, request):
        if request.user.is_superuser:
            return super(MessageAdmin, self).get_queryset(request)
        else:
